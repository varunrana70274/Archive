...
class EditVideoIntro extends Component {
  constructor(props) {
    super(props);
  ...
  uploadIntroVideo = async () => {
    ...
    if(this.state.x){formData.append("x", this.state.x)}
    if (this.state.y){formData.append("y", this.state.y)}
    if(this.state.text){formData.append("text", this.state.text);}
    // if(this.state.text){
    //   formData.append("video_text", this.state.text);
    // }
    this.setState({ loading: true });
    API.uploadUserIntroUpdate(formData)
      .then(async (response) => {
  ...
  render() {
    const { media, loading, textEnable, text,loading2 } = this.state;
      ...
      <View style={[styles.container]}>
        ...
        {!this.state.audionAdded ? (
          <>
            ...
            }
            {
              <TouchableOpacity
                onPress={async() => {
                  this.setState({mute:true,shouldPlay:false})
                 await this.paused.pauseAsync()
                  RNVoiceRecorder.Record({
                    onDone: (path) => {
                      console.log('path',path);
                      this.setState({ audioFile: 'file:///'+path,audioClick:true,soundEdit:true,play:true,audionAdded:true,mute:false });
                      // RNVoiceRecorder.Play(path)
                      this.paused?.playAsync?.()
                    },
          ...
          <TouchableOpacity
            onPress={
              () => {
                this.setState({ soundEdit: !this.state.soundEdit,mute:true });
              }
        ...
        {/* { (
          ...
          volume={this.state.voloumeVideo}
            ...
            source={this.props?.route?.params?.intro_video}
...
export default connect(null, { updateUser })(EditVideoIntro);
