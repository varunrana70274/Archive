...
const DEFAULT_TINT_COLOR: string = Colors.primary;
const DEFAULT_OFF_TINT_COLOR: string ='#1B1E29';
  ...
  onKeyPress = (
    ...
    if (e.nativeEvent.key === "Backspace" && i !== 0) {
      if (!val.length && otpText[i - 1].length === inputCellLength) {
        this.setState(
          (prevState) => {
            let { otpText: otpTextPrev } = prevState;

            otpTextPrev[i - 1] = otpTextPrev[i - 1]
              .split("")
              .splice(0, otpTextPrev[i - 1].length - 1)
              .join("");
  ...
  render() {
    ...
    for (let i = 0; i < inputCount; i += 1) {
      ...
      const inputStyle = [
        ...
        textInputStyle,
        {
          borderColor: this.props.error?'red':_offTintColor,
        },
      ];

      if (focusedInput === i) {
        inputStyle.push({
          borderColor:this.props.error?'red': _tintColor,
        });
      }
