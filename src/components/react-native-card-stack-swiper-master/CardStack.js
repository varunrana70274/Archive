    ...
    this._panResponder = PanResponder.create({
      ...
      onMoveShouldSetPanResponder: (evt, gestureState) => {
        ...
        if (!this.props.verticalSwipe && isVerticalSwipe) {
         if(gestureState.dy>Dimensions.get('window').height-Dimensions.get('window').height*0.4){
          ...
          if(this?.state?.cards?.length>0){
            console.log('this?.state?.cards[0]',this?.state?.cards[0]);
            this?.state?.cards[index]?.props?.bottomDownSwipe()
          }
         }
          return false
        }
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (((Math.abs(gestureState.dx) > horizontalThreshold) ||
          ...
          if (swipeDirection < 0 && !disableTopSwipe) {
            ...
            // index of the swiped card
            const index = (loop) ? this.mod(nextCard - 2, cards.length) : nextCard - 2;
            this.state.cards[index].props.onSwipeStartBottom();
            this.props.onSwipeStartBottom();
            this._resetCard()
          }
          else {
            this._resetCard();
  ...
  render() {
    ...
    return (
      <View {...this._panResponder.panHandlers} style={[{ position: 'relative' }, this.props.style]}>
        ...
        {/* <View style={{backgroundColor:'red',position:'absolute',zIndex:99999,bottom:0,height:100,width:100}}> */}
          {/* {this.props.layer} */}
        {/* </View> */}
