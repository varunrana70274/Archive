...
class UnoteCameraPreview extends Component {
  constructor(props) {
    super(props);
  ...
  uploadUnote = async () => {
    ...
    if (Platform.OS == "ios") {
      ...
      let path =
        ...
        console.log('pathpathpath4',path,this.state.media.uri);
    ...
    API.sendUnote(this.state.recipient_id, formData)
      .then(async (response) => {
    ...
    return (
      <View style={styles.container}>
        ...
        >
          ...
          <TouchableOpacity onPress={this.uploadUnote}>
            <Text primary>Send Unote</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
...
export default connect(null, { updateUser })(UnoteCameraPreview);
